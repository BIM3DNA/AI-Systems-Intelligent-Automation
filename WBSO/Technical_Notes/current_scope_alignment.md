...
## Historical pre-commit M3F scope checkpoint - 2026-09-23
...
Required live matrix complete; no package-introduced defect identified. No scope
expansion. Curve/tap retest unclaimed; closed End semantics and static regression
remain authoritative. Next: review, then explicitly authorize commit/push.
Evidence/Daily Log/KC IDs and hours PENDING; no allocation or closure claim.
...
Scope proven so far: A01/A02 on one straight OVAL duct only. A03/A04, no-tool
answer and electrical/mixed boundaries remain pending; no broad HVAC live acceptance.
...
Scope is controlled exposure of the four closed HVAC-RO-001 A01-A04 actions,
retaining the four PIPING-RO-001 A01-A04 tools: eight read-only tools total.
No Electrical AI tool, mutation tool, arbitrary dispatch, autonomous multi-tool
loop, AutoCAD, ScanAI integration or new specialty orchestration.
Closed HVAC/Piping semantics and HVAC-QA-009 End-versus-Curve behavior remain
unchanged. Existing ExternalEvent/headless ownership and stale-context guards
remain; maximum one execution. Provider projection transports deterministic data,
not an alternate domain evaluator. Catalog remains 237; manifests unchanged.

Implementation/static checkpoint only, not live acceptance. LIVE-M3D-01..07 are
NOT STARTED / PENDING. Next: review this documentation, then user-run live matrix
in BIMCode_Provider/M3D.md and test_plan.md. Evidence ID, Daily Log ID, KC ID and
hours: PENDING; none allocated. No closure or live PASS claimed.
...
### 2026-05-19 MEP-WR-002 and MEP-WR-003 scope alignment

MEP-WR-002 validates rollback-only use of Revit's pipe split API. It uses `TransactionGroup` and `PlumbingUtils.BreakCurve` only after explicit `ROLLBACK-TEST-OK`, and rolls the transaction group back so no persistent model mutation remains.

MEP-WR-003 validates the first persistent reviewed apply path. The apply is deliberately limited to exactly one candidate that must be generated by MEP-WR-001, successfully rollback-tested by MEP-WR-002, explicitly selected by candidate number or pipe id, and explicitly confirmed with `PERSISTENT-SPLIT-OK`.

Both features support the approved direction of moving from deterministic reporting to controlled automation through staged evidence:

- dry-run candidates precede rollback tests
- rollback tests precede persistent apply
- persistent apply is single-candidate only
- no batch apply exists
- generic `apply reviewed action` and `execute latest proposal` prompts remain blocked by MEP-ACT-002
