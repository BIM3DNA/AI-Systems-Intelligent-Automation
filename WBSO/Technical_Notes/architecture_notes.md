...
## Historical pre-commit M3F architecture checkpoint - 2026-09-23
...
Live single-tool parity, no-tool general answer, Conduit rejection and all three
mixed-specialty boundaries PASS. No architectural correction required.
...
All LIVE-M3D-01 through LIVE-M3D-07 PASS. No architecture correction required.
Current static results: validation_summary.md. Full evidence: evidence_reference.md.
...
HVAC-QA-009 remains abnormal physical End count; valid physical Curve/tap
connectors must not create false abnormal-End issues. No HVAC domain logic is
recomputed by provider projection. The restart is a deployment/runtime-loading
observation, not evidence requiring an execution-architecture change.
...
## 2026-08-05 - ELECTRICAL-DISC-001 Read-Only Electrical Selection Discovery

ELECTRICAL-DISC-001 adds one bounded deterministic discovery model over the user's current active-document selection. The architecture reuses guarded MEP-RO identity and parameter reads but does not establish ELECTRICAL-RO-001 production behavior. Eight family-instance categories are discovery candidates; conduit, cable tray, fittings, wire, circuits, links, and unrelated elements remain explicit unsupported or separately classified scope.

The initial implementation treated all connector/API exceptions as element-wide unreadable evidence. Live Lighting Fixture, Electrical Fixture, Electrical Equipment, and Conduit tests therefore produced false PARTIAL results when optional fields were absent or physical fields did not apply to Logical, Surface, MasterSurface, or conduit-domain connectors.

The corrected architecture separates normalized availability from severity. Connector type and domain determine applicability; only failures that prevent required identity, enumeration, or meaningful connector/system classification contribute to PARTIAL. Logical connectors retain AllRefs/system-owner semantics without requiring physical geometry. Surface/MasterSurface retain exact type and conduit interface evidence. DomainCableTrayConduit electrical quantities are NOT_APPLICABLE unless exposed. Room/Space use a stable created-phase lookup, Level retains ID and resolved name, and raw IronPython indexer objects are excluded.

Electrical quantities retain raw internal diagnostics and use guarded Revit UnitUtils conversion to V, VA, and W where available; power factor remains a unitless ratio and poles an integer. No conversion factor is guessed. ElectricalSystem records preserve zero/one/multiple assignments and distinguish selected-panel `BASE_EQUIPMENT` downstream branches from selected-element `LOAD` upstream/load relationships where observable.

Corrected runtime evidence provisionally supports Lighting Fixtures and Electrical Fixtures, conditionally supports Electrical Equipment because of mixed connector and multi-system semantics, and excludes Conduit from the initial device-oriented production scope. Four exact discovery routes are uniquely owned; global fuzzy dispatch is unchanged and future production routes remain unregistered. Evidence: EV-AI-357; KC-050.

## 2026-08-08 - ELECTRICAL-RO-001 Read-Only Electrical Selection Architecture

ELECTRICAL-RO-001 converts the prior discovery findings into four bounded production reports without changing the closed discovery handlers. Exact production routes dispatch after ELECTRICAL-DISC exact ownership and before generic MEP/fuzzy fallback, resolving the prior production-like wording collision narrowly rather than rewriting global routing.

The architecture uses two profiles. `DEVICE_PROFILE` covers Lighting Fixtures and Electrical Fixtures, where an assigned load circuit is the common authoritative relationship. `EQUIPMENT_PROFILE` covers Electrical Equipment, where zero systems, one upstream feeder, multiple downstream branches, or mixed upstream/downstream roles can all be valid states. Multiple systems are not inherently contradictory: a panel can be `LOAD` on an upstream feeder and `BASE_EQUIPMENT` for distinct downstream circuits.

Connector interpretation is applicability-driven. Only physical connector types in `DomainElectrical` are `PHYSICAL_ELECTRICAL`. Logical connectors retain circuit-reference semantics but are not physical. Surface and MasterSurface connectors are interfaces; `DomainCableTrayConduit` quantities and geometry may be `NOT_APPLICABLE` rather than unreadable. This distinction prevents optional/non-applicable data from forcing false PARTIAL reports.

System quantities reuse guarded discovery normalization to V, VA, W, unitless power-factor ratio, and integer poles where reliable. Circuit/system records are authoritative; connector quantities are supplementary. Stable QA is deliberately conservative: checks `ELECTRICAL-QA-001` through `011` cover scope, required read integrity, device assignment identity, equipment role readability, role consistency, normalized quantities, and connector-read completeness. Connector-count/open-state and speculative phase/balancing/demand/pole/Room/Space/Mark rules are excluded.

Context Suggestions uses `min(18, 6 + 4 x eligible specialty count)`. Supported electrical devices/equipment activate one electrical gate; unsupported Conduit alone does not. All reports remain read-only, workflow-anchor ineligible, strict-QA-source ineligible, and evidence-stage false. Live validation is in progress; panelboard A03/A04 and the remaining mixed/failure/cap matrix are not yet complete. Source remains uncommitted and unpushed. Evidence: EV-AI-358; KC-051.

### Final Architecture Validation - 2026-08-12

Final live evidence confirmed both profiles and their boundary cases: assigned and unassigned devices, zero-system and multi-system equipment, simultaneous upstream `LOAD` and downstream `BASE_EQUIPMENT`, mixed supported profiles, and supported-plus-unsupported PARTIAL scope. P108 A03/A04 showed coherent eight-system role distribution and GREEN QA; duplicate visible circuit number 2 remained non-contradictory because system IDs, panels, roles, and relationships differed. The unassigned Lighting Fixture produced only `ELECTRICAL-QA-003`, with QA-004/005 correctly not applicable.

The specialty-cap formula passed at 6/10/14/18 for normal, one-, two-, and three-specialty contexts, including Visual Preview. `DEVICE_MULTI_SYSTEM_REVIEW` was not practically reproduced; failure/cap and additional category paths remain explicit non-blocking limitations. The final static/Git audit found no routing, mutation, workflow, or provider regression. Evidence: EV-AI-359; KC-051.
...
Supported discovery scope is deliberately limited to the closed production scope: rigid Pipe; rigid non-placeholder Duct; and `OST_LightingFixtures`, `OST_ElectricalFixtures`, and `OST_ElectricalEquipment`. No additional electrical category is supported. Other active-view candidates may contribute only bounded generic-only/unsupported comparison context.

Each comparison family declares `ONE_TO_ONE`, `CONDITIONAL`, `COVERAGE_ONLY`, or `NOT_ONE_TO_ONE`. Only declared comparable mappings can create agreement or disagreement. `COVERAGE_ONLY` and `NOT_ONE_TO_ONE` evidence remains visible but does not increment disagreement; unmapped specialty issues are not promoted automatically to specialty-only disagreements. Consequently, an OK result means the declared comparable mappings completed without disagreement, not that specialty QA found no issues.

Processing caps are 90 supported specialty elements total, 30 Pipe, 30 Duct, 30 Electrical, and 50 generic candidates per comparison family. Display caps are 100 comparison rows, 75 disagreement rows, and 50 warnings. Runtime testing led to separate `processing_omitted_count` and `display_omitted_count`, distinct `SUPPORTED_SPECIALTY_PROCESSING_CAP` and `GENERIC_ONLY_UNSUPPORTED_PROCESSING_CAP` profiles, and phase-specific timing denominators. Classification continues to depend on processing state, not display truncation.

An empty/unsupported/not-applicable active view supplies no negative production evidence. Those NOT_READY paths, and FAILED paths with no completed semantic evaluation, use `MORE_RUNTIME_EVIDENCE_REQUIRED` unless independent substantive evidence justifies another non-negative recommendation. `NOT_RECOMMENDED_AT_ACTIVE_VIEW_SCALE` is reserved for completed or substantive evidence of unsuitability.

### Completed Planned Runtime Matrix and Final Audit - 2026-08-25
...
Exactly five subscriptions: Application.DocumentOpened, DocumentCreated,
DocumentClosed; UIApplication.ViewActivated, SelectionChanged. Open/create/close
reset show_pending; close clears context. View activation or queued read-only
refresh calls GetDockablePane(stable UUID).Show when a valid document is active.
Success clears pending; no-document retains pending; normal views do not repeat
Show. UUID: `aa6b23d4-f8e3-4b2f-9ad7-de9e05bfb5e4`.

Startup uses PaneIsRegistered, retaining panel/provider/delegates/ExternalEvent
under BIMCODE_AI_PANE_M1_SESSION. Right docking; VisibleByDefault=true. The
ribbon opens only. SelectionChanged reads args.GetSelectedElements().Count only;
explicit Refresh uses a read-only ExternalEvent for document/view/type/count.
No timers, polling, Idling, background API thread, mutation or evidence write.
