...
Comparison relations are `ONE_TO_ONE`, `CONDITIONAL`, `COVERAGE_ONLY`, and `NOT_ONE_TO_ONE`. Coverage-only and non-one-to-one observations do not increment disagreement, and unmapped specialty QA evidence does not automatically become a specialty-only disagreement. Processing is bounded to 90 supported elements total and 30 per specialty; generic comparison candidates are capped at 50 per family, with display caps of 100 comparison rows, 75 disagreement rows, and 50 warnings.
