...
| M-006 | planner provider state via `Model_Service/ModelService.py` + `chatgpt_service.py` | provider state model | reports local/cloud planner availability and request-failure state to the AI Agent UI | active | cloud path requires `OPENAI_API_KEY` from environment |
